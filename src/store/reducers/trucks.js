...
const submitTruckInputsSuccess = (state,action) => {
  ...
  const removeTruck = (state, action) => {
    return updateObject(state,{
      loading: false,
      trucks: state.trucks.filter(item => item.YearlyStrippingRate !== action.payload.YearlyStrippingRate),
    })
  }
...
const reducer = (state = initialState, action) => {
  switch(action.type) {
    ...
    case actionTypes.REBUILD_TRUCKS_FROM_LOCAL_STORAGE:
      return {...state, trucks: action.payload.trucks.map(item => item.truckData)};
    case actionTypes.SUBMIT_TRUCK_INPUTS_FAIL: return submitTruckInputsFail(state,action);
    case actionTypes.SUBMIT_INPUTS_START:return submitTruckInputsStart(state,action);
    case actionTypes.REMOVE_TRUCK: return removeTruck(state,action);
    default:
      return state;
  }
