...
export const removeNode = (selectedNode) => {
  const nodesFromLocalStorage = JSON.parse(localStorage.getItem("nodes"))
    .map((item) => item.nodeData)
    .filter((item) => item.Nodes !== selectedNode.Nodes)
    .map((item) => ({ nodeData: item }));
  ...
  const pathsFromLocalStorage = JSON.parse(
    ...
    (item) =>
      !(
        item.FirstNode === selectedNode.Nodes ||
        item.SecondNode === selectedNode.Nodes
      ) && item
  );
...
export const rebuildNodesFromLocalStorage = () => {
  ...
  return nodes
    ? {
        type: actionTypes.REBUILD_NODES_FROM_LOCAL_STORAGE,
        payload: { nodes, types: nodeTypes },
      }
    : {
        type: actionTypes.REBUILD_NODES_FROM_LOCAL_STORAGE,
        payload: { nodes: [], types: [] },
