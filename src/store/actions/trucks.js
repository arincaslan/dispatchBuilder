...
export const removeTruck = (selectedTruck) => {
  const truckDataFromLocalStorage = JSON.parse(localStorage.getItem("trucks")).map(item => item.truckData).filter(item => item.YearlyStrippingRate !== selectedTruck.YearlyStrippingRate).map(item => ({truckData: item}));
...
export const submitTruckInputs = (truckData, token) => {
  return dispatch => {
    ...
    dispatch({type: actionTypes.SUBMIT_TRUCK_INPUTS_SUCCESS, payload: truckData})
...
export const rebuildTrucksFromLocalStorage = () => {
  ...
  return trucks  ? {type: actionTypes.REBUILD_TRUCKS_FROM_LOCAL_STORAGE, payload: {trucks}} : {type: actionTypes.REBUILD_TRUCKS_FROM_LOCAL_STORAGE, payload: {trucks: []}};
}
